...
int main() {
  ...
  // Load another texture image, with transparency (alpha)
  int width2{0};
  int height2{0};
  int nrChannels2{0};
  // TODO: harcoded relative path
  ...
  */
  ...
  if (data2 != nullptr)
  {
    ...
    glTexImage2D(
      GL_TEXTURE_2D,  // texture target, generated texture will only affect textures bound to this target
      0,  // mipmap level we want this texture to be generated, we leave the default
      GL_RGBA,  // storage format of the resulting texture in OpenGL, we have A (alpha, transparency)
      width2,  // of the restulting texture
      height2, // of the resulting texture
      0,  // legacy stuff, should always be 0
      GL_RGBA,  // format of the source image
      GL_UNSIGNED_BYTE,  // format of the source image
      data2 // actual source image data
    );
    // now the currently bound texture object as the texture image bound to it
