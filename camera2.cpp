...
int main() {
  ...
  glm::vec3 camera_right{glm::normalize(glm::cross(up, camera_opposite_direction))};
  glm::vec3 camera_up{glm::normalize(glm::cross(camera_opposite_direction, camera_right))};
  ...
  while(!glfwWindowShouldClose(window))
  {
    ...
    // Let the camera circle around
    const float radius{10.0f};
    auto cam_pos_x{sin(glfwGetTime()) * radius};
    auto cam_pos_y{cos(glfwGetTime()) * radius};

    camera_pos = glm::vec3(cam_pos_x, 0., cam_pos_y);
